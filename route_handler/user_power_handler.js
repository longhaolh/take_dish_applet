...
/*
* 获取用户信息接口
* 根据token中的id获取用户信息 req.auth为解析后的token信息
* */
...
exports.uploadAvatar = (req,res)=>{
    db.query("UPDATE users SET avatar = ? WHERE id = ?",[req.body.avatar,req.auth.id],(err,data)=>{
        if(err){
            res.status(500).cc(err)
        }else{
            res.send({stauts:0,message:'头像上传完成'})
        }
    })
}